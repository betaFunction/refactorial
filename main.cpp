...
int main(int argc, char **argv) {
  ...
  for (auto configSectionIter = config.begin();
       configSectionIter != config.end(); ++configSectionIter) {
    ...
    if (configSection["Files"])
      ...
      for (auto iter = compileCommands.begin(); iter != compileCommands.end();
           ++iter) {
    ...
    if (!configSection["Transforms"]) {
      llvm::errs()
          << "No transforms specified in this configuration section:\n";
      llvm::errs() << YAML::Dump(configSection) << "\n";
    }

    // load up the compilation database
    llvm::OwningPtr<tooling::CompilationDatabase> Compilations(
        tooling::CompilationDatabase::loadFromDirectory(".", errorMessage));
    ...
    for (auto iter = configSection["Transforms"].begin();
         iter != configSection["Transforms"].end(); iter++) {

      llvm::errs() << iter->first.as<string>() + "Transform"
                   << "\n";
      rt.run(new TransformFactory(
          TransformRegistry::get()[iter->first.as<string>() + "Transform"]));
    }
  }
  return 0;
}
