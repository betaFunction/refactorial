...
void ExtractParameterTransform::HandleTranslationUnit(ASTContext &C) {
  auto extractSpec =
      TransformRegistry::get().config["ExtractParameter"].begin();
  ...
  for (auto RI = rewrites.begin(), RE = rewrites.end(); RI != RE; ++RI) {
    ...
    if (FN->getNumParams() > 0) {
      ...
      SourceLocation lParenLoc =
          dyn_cast<FunctionTypeLoc>(&TL)->getLocalRangeBegin();
      insertionLoc = getLocForEndOfToken(lParenLoc);
    }
    vector<YAML::Node> transformData = TransformRegistry::get()
                                           .config["ExtractParameter"]
                                           .as<vector<YAML::Node> >();
    for (auto CI = transformData.begin(), CE = transformData.end(); CI != CE;
         ++CI) {
      YAML::Node node = *CI;
      if (node["method"].as<string>() == FN->getQualifiedNameAsString()) {
        for (auto VI = values.begin(), VE = values.end(); VI != VE; ++VI) {
          if (node["variable"].as<string>() == (*VI)->getNameAsString()) {
            string str = (*VI)->getType().getAsString() + " " +
                         (*VI)->getNameAsString() + " = " +
...
void ExtractParameterTransform::removeDecl(const Stmt *stmt,
                                           const VarDecl *decl) {
  assert(isa<CompoundStmt>(stmt));
  const CompoundStmt *cstmt = dyn_cast<CompoundStmt>(stmt);
  for (auto substmt = cstmt->body_begin(); substmt != cstmt->body_end();
       ++substmt) {
        ...
        if (*test_decl == decl) {
          ...
          if (DGR.isSingleDecl()) {
            ...
            if (test_decl == DGR.begin()) {
              // first decl includes type--we must remove it, and add a comma at
              // the end
              TypeLoc TL = decl->getTypeSourceInfo()->getTypeLoc();
              rangeToRemove.setBegin(getLocForEndOfToken(TL.getLocEnd()));
              rangeToRemove.setEnd(Lexer::findLocationAfterToken(
                  rangeToRemove.getEnd(), clang::tok::comma,
                  sema->getSourceManager(), sema->getLangOpts(),
                  false).getLocWithOffset(-1));
            } else {
              // any other decl--we must add a comma at the beginning
              rangeToRemove.setBegin(
                  getLocForEndOfToken((decl - 1)->getSourceRange().getEnd()));
            }
          }
