...
void Transform::InitializeSema(Sema &s) { sema = &s; }

void Transform::insert(SourceLocation loc, string text) {
  TransformRegistry::get().replacements->push_back(
      Replacement(sema->getSourceManager(),
                  CharSourceRange(SourceRange(loc, loc), false), text));
}

void Transform::replace(SourceRange range, string text) {
  TransformRegistry::get().replacements->push_back(Replacement(
      sema->getSourceManager(), CharSourceRange(range, true), text));
}
...
const transform_creator TransformRegistry::
...
protected:
  ...
  virtual bool BeginInvocation(CompilerInstance &CI) {
    CI.getHeaderSearchOpts().AddPath("/usr/local/lib/clang/3.2/include",
