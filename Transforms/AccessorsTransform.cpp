...
public:
  ...
  void collect(const DeclContext *ns_decl) {
    for (auto subdecl = ns_decl->decls_begin(); subdecl != ns_decl->decls_end();
         subdecl++) {
      /*
      ...
      if (const CXXRecordDecl *rc_decl = dyn_cast<CXXRecordDecl>(*subdecl)) {
        for (auto member = rc_decl->field_begin();
             member != rc_decl->field_end(); member++)
          if (find(fields.begin(), fields.end(),
                   member->getQualifiedNameAsString()) != fields.end())
  ...
  void rewrite(const BinaryOperator *bin_op, const ParentMap &PM) {
    ...
    for (auto iter = fieldRanges.begin(); iter != fieldRanges.end(); ++iter) {
      if (lhs_expr->getMemberDecl() == iter->second) {
        ...
        // lhs_expr->getBase()->printPretty(base_sstr, *ctx, 0,
        // PrintingPolicy(ctx->getLangOpts()));

        string getterName =
            "get" + lhs_expr->getMemberDecl()->getNameAsString();
        getterName[3] = toupper(getterName[3]);
        string setterName =
            "set" + lhs_expr->getMemberDecl()->getNameAsString();
        ...
        if (bin_op->isCompoundAssignmentOp()) {
          if (bin_op != top_stmt_within_compound) {
            ...
            sstr << BinaryOperator::getOpcodeStr(
                        BinaryOperator::getOpForCompoundAssignment(
                            bin_op->getOpcode())) << " ";
            // bin_op->getRHS()->printPretty(sstr, *ctx, 0,
            // PrintingPolicy(ctx->getLangOpts()));
            collect(bin_op->getRHS(), PM);
            sstr << " );\n";
            insert(top_stmt_within_compound->getLocStart(), sstr.str());
            replace(bin_op->getSourceRange(),
  ...
  void rewrite(const UnaryOperator *un_op, const ParentMap &PM) {
    ...
    for (auto iter = fieldRanges.begin(); iter != fieldRanges.end(); ++iter) {
      if (sub_expr->getMemberDecl() == iter->second) {
        ...
        // sub_expr->getBase()->printPretty(base_sstr, *ctx, 0,
        // PrintingPolicy(ctx->getLangOpts()));
        base_str = base_sstr.str();

        string getterName =
            "get" + sub_expr->getMemberDecl()->getNameAsString();
        getterName[3] = toupper(getterName[3]);
        string setterName =
            "set" + sub_expr->getMemberDecl()->getNameAsString();
        setterName[3] = toupper(setterName[3]);
        if (!un_op->isIncrementDecrementOp()) {
          ...
          if (const IfStmt *if_stmt =
                  dyn_cast<IfStmt>(PM.getParent(top_stmt_or_compound))) {
            if (if_stmt->getThen() == top_stmt_or_compound ||
              ...
              if (onlyExpr)
                ...
                if (un_op->isPrefix()) {
                  ...
                  insert(findLocAfterSemi(un_op->getLocEnd()),
                         incrStmt + ";\n");
                }
                if (onlyStmt)
          ...
          } else if (const ForStmt *for_stmt = dyn_cast<ForStmt>(
                         PM.getParent(top_stmt_or_compound))) {
            if (for_stmt->getBody() == top_stmt_or_compound) {
              ...
              if (onlyExpr)
                ...
                if (un_op->isPostfix())
                  // not sure if this is legit, but i can't think of a better
                  // way
          ...
          /*
            ...
            || (forStmt =
            dyn_cast<ForStmt>(PM.getParent(top_stmt_within_compound))
          ...
          if (needToInsertBraces) {
            ...
            SourceLocation locAfterSemi =
                findLocAfterToken(top_stmt_or_compound->getLocEnd(), tok::semi);
            insert(locAfterSemi, "}\n");
          }
        }
      }
  ...
  void rewrite(const MemberExpr *mem_expr, const ParentMap &PM) {
    for (auto iter = fieldRanges.begin(); iter != fieldRanges.end(); ++iter) {
      if (mem_expr->getMemberDecl() == iter->second) {
        ...
        string stmts_str, base_str;
        llvm::raw_string_ostream sstr(stmts_str), base_sstr(base_str);
        // mem_expr->getBase()->printPretty(base_sstr, *ctx, 0,
        // PrintingPolicy(ctx->getLangOpts()));

        string getterName =
            "get" + mem_expr->getMemberDecl()->getNameAsString();
        getterName[3] = toupper(getterName[3]);
        string setterName =
            "set" + mem_expr->getMemberDecl()->getNameAsString();
        setterName[3] = toupper(setterName[3]);
        sstr << base_sstr.str() << "." << getterName << "()";
  ...
  void collect(const Stmt *stmt, const ParentMap &PM) {
    if (const BinaryOperator *bin_op = dyn_cast<BinaryOperator>(stmt))
      ...
      for (auto child = stmt->child_begin(); child != stmt->child_end();
           ++child) {
  ...
  void insertAccessors() {
    for (auto iter = fieldRanges.begin(); iter != fieldRanges.end(); ++iter) {
      ...
      sstr << type << " &get" << fnname << "()  { return " << varname
           << "; };\n";
      // setter
      sstr << "void set" << fnname << "(" << ctype << "& _" << varname << ") { "
           << varname << " = _" << varname << "; };\n";

      bool hasUserDefinedMethods = false;
      for (auto iter = parent->method_begin(); iter != parent->method_end();
           ++iter)
