    ...
    if (auto D = dyn_cast<ClassTemplateDecl>(*I)) {
      ...
      if (auto CRD = dyn_cast<CXXRecordDecl>(TD)) {
        ...
        if (CRD->hasDefinition()) {
          for (auto BI = CRD->bases_begin(), BE = CRD->bases_end(); BI != BE;
               ++BI) {
          ...
          for (auto FI = CRD->friend_begin(), FE = CRD->friend_end(); FI != FE;
               ++FI) {
...
void TypeRenameTransform::processFunctionDecl(FunctionDecl *D) {
  ...
  if (auto DD = dyn_cast<CXXDestructorDecl>(D)) {
    ...
    if (BL.isValid() && P->getLocation() != BL &&
      ...
      SourceLocation EL = Lexer::findLocationAfterToken(
          BL, tok::raw_identifier, sema->getSourceManager(),
          sema->getLangOpts(), false);

      // TODO: Find the right way to do this -- consider this a hack

      // llvm::errs() << indent() << "dtor at: " << loc(BL) << ", locStart: " <<
      // loc(DD->getLocStart())
      //   << ", nameAsString: " << P->getNameAsString() << ", len: " <<
      // P->getNameAsString().size()
      //   << ", DD nameAsString: " << DD->getNameAsString() << ", len: " <<
      // DD->getNameAsString().size()
      //   << "\n";
